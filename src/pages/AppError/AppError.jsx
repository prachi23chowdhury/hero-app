import React from 'react';
import { useNavigate } from 'react-router';
import appError from "../../assets/App-Error.png"
import Navbar from '../../components/Navbar';
import Footer from '../../components/Footer';
import Title from '../../components/Title';

const AppError = () => {
  Title('AppError | HeroApp');
  const navigate = useNavigate();

  return (
    <div>
        <Navbar></Navbar>
       <div className="flex flex-col items-center justify-start min-h-screen bg-gray-100 px-5 pt-5 text-center">
  <img src={appError} alt="404 Error" className="w-80 md:w-96 mb-4" />
  <h1 className="text-3xl md:text-4xl font-bold mb-2 text-gray-900">OPPS!! APP NOT FOUND</h1>
  <p className="text-gray-600 mb-4">The App you are requesting is not found on our system.  please try another apps</p>
  <button
    onClick={() => navigate(-1)}
    className="bg-primary text-white font-semibold px-6 py-2 rounded
               shadow-md hover:bg-purple-700 hover:shadow-lg transition-all duration-300"
  >
    Go Back!
  </button>
</div>

    <Footer></Footer>
    </div>
    );
};

export default AppError;